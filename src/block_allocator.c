...
b3BlockAllocator b3CreateBlockAllocator( int elementSize, int initialCount )
{
	...
	// Blocks come from b3Alloc, so a stride that is a multiple of B3_BLOCK_ALIGNMENT aligns all elements.
	elementSize = ( ( elementSize - 1 ) | ( B3_BLOCK_ALIGNMENT - 1 ) ) + 1;
...
void b3FreeElement( b3BlockAllocator* allocator, void* element )
{
	...
	B3_ASSERT( ( (uintptr_t)element & ( B3_BLOCK_ALIGNMENT - 1 ) ) == 0 );
